...
import platform
import time
print("[*] ZCRACK INSTALLING..............")
def header():
    ascii_banner = pyfiglet.figlet_format("🔥ZCRACKER🔥").upper()
    print(colored(ascii_banner.rstrip("\n"), 'yellow', attrs=['bold']))
    print(colored("                                Made By: AbuTalha    \n", 'yellow', attrs=['bold']))
    print(colored("                                Version: 1    \n", 'red', attrs=['bold']))
    print(colored("                                Team:FaceLessOfficialBd    \n", 'green', attrs=['bold']))
    return
...
def linuxpdf():
    os.system("clear")
    header()
    zip_filename = input(termcolor.colored("[*] Enter Path Of Your zip file And Crack:- ", 'red'))
